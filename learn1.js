...
function delgoogle() {
	  ...
	  if(zkdurldomain.indexOf('google.com') > 0  && zkdurldomain.indexOf('/search?') > 0){
    	   ...
    	   if ( $('#bingconfig_btn').length>0){
  	   	    ...
  	   	    $('#hdtb-tls').before('<a id="bingconfig_btn" class="gb_Sc" style="border: 1px solid transparent;   text-align: center;    border-radius: 2px;    line-height: 19px;    cursor: pointer;    margin-left: -1px;    padding: 4px 11px;  margin-right: -11px;"  href="'+configm+'"  >控制面板</a>');
  	   }
...
let urldofavicon = "";
if( urldomain[2] ) {
 urldofavicon="https://favicon.yandex.net/favicon/v2/"+urldomain[2]+"?size=32";

	}
$(this).attr("zlogo","1");
$(this).find("h3").prepend('<div style="border-radius: 1%;display: inline-block; width: 20px; height: 20px; position: relative; top: 3px;background: rgba(0, 0, 0, 0);margin-right: 8px;"><img  style="max-width:24px;" src="'+urldofavicon+'"  ></div>');
...
$("div[data-kwurled='1']").css("background-color",GM_getValue('allconfig').baidu.baidueyecolor);
   }
  }
